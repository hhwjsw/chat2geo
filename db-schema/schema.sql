...
CREATE OR REPLACE FUNCTION "public"."search_documents_by_similarity"("query_embedding" "public"."vector", "match_count" integer DEFAULT NULL::integer, "filter" "jsonb" DEFAULT '{}'::"jsonb") 
...
CREATE OR REPLACE FUNCTION "public"."search_documents_by_similarity"("query_embedding" "public"."vector", "match_count" integer DEFAULT NULL::integer, "owner_uuid" "uuid" DEFAULT NULL::"uuid", "metadata_filter" "jsonb" DEFAULT '{}'::"jsonb") 
RETURNS TABLE("id" bigint, "content" "text", "metadata" "jsonb")
    LANGUAGE "plpgsql"
    AS $$
BEGIN
...
-- 更新搜索函数以使用 websearch_to_tsquery
CREATE OR REPLACE FUNCTION "public"."search_gee_datasets_ft"("query" "text") 
RETURNS TABLE("id" integer, "dataset_id" "text", "asset_url" "text", "type" "text", "start_date" "date", "end_date" "date", "title" "text", "rank" real)
    LANGUAGE "plpgsql"
    AS $$
BEGIN
  RETURN QUERY
  SELECT
    ...
    gd.end_date,
    gd.title,
    ts_rank(gd.search_vector, websearch_to_tsquery('simple', query)) AS rank
  FROM public.gee_datasets gd
  WHERE gd.search_vector @@ websearch_to_tsquery('simple', query)
  ORDER BY rank DESC;
END;
$$;
...
ALTER TABLE ONLY "public"."document_files" ADD CONSTRAINT "document_files_file_path_key" UNIQUE ("file_path");
ALTER TABLE ONLY "public"."document_files" ADD CONSTRAINT "document_files_owner_name_key" UNIQUE ("owner", "name");
ALTER TABLE ONLY "public"."document_files" ADD CONSTRAINT "document_files_pkey" PRIMARY KEY ("id");
ALTER TABLE ONLY "public"."drafted_reports" ADD CONSTRAINT "drafted_reports_pkey" PRIMARY KEY ("id");
ALTER TABLE ONLY "public"."embeddings" ADD CONSTRAINT "embeddings_pkey" PRIMARY KEY ("id");
ALTER TABLE ONLY "public"."gee_datasets" ADD CONSTRAINT "gee_datasets_pkey" PRIMARY KEY ("id");
...
ALTER TABLE ONLY "public"."user_usage" ADD CONSTRAINT "user_usage_pkey" PRIMARY KEY ("id");
...
-- 为 user_roles 表创建触发器
DROP TRIGGER IF EXISTS user_roles_after_insert ON public.user_roles;
CREATE TRIGGER user_roles_after_insert
  AFTER INSERT ON public.user_roles
  FOR EACH ROW
  EXECUTE FUNCTION public.create_user_usage_row();

-- 为 document_files 表创建触发器
DROP TRIGGER IF EXISTS trigger_update_docs_count ON public.document_files;
CREATE TRIGGER trigger_update_docs_count
  AFTER INSERT OR DELETE OR UPDATE OF owner ON public.document_files
  FOR EACH ROW
  EXECUTE FUNCTION public.update_user_usage_docs_count();

-- 添加外键约束
ALTER TABLE ONLY "public"."chats" ADD CONSTRAINT "chats_user_id_fkey" FOREIGN KEY ("userId") REFERENCES "public"."user_roles"("id") ON DELETE CASCADE;
ALTER TABLE ONLY "public"."document_files" ADD CONSTRAINT "document_files_owner_fkey" FOREIGN KEY ("owner") REFERENCES "public"."user_roles"("id") ON DELETE CASCADE;
ALTER TABLE ONLY "public"."drafted_reports" ADD CONSTRAINT "drafted_reports_user_id_fkey" FOREIGN KEY ("userId") REFERENCES "public"."user_roles"("id") ON DELETE CASCADE;
ALTER TABLE ONLY "public"."embeddings" ADD CONSTRAINT "embeddings_file_id_fkey" FOREIGN KEY ("file_id") REFERENCES "public"."document_files"("id") ON DELETE CASCADE;
ALTER TABLE ONLY "public"."messages" ADD CONSTRAINT "messages_chat_id_fkey" FOREIGN KEY ("chatId") REFERENCES "public"."chats"("id") ON DELETE CASCADE;
ALTER TABLE ONLY "public"."messages" ADD CONSTRAINT "messages_drafted_report_id_fkey" FOREIGN KEY ("draftedReportId") REFERENCES "public"."drafted_reports"("id") ON DELETE CASCADE;
ALTER TABLE ONLY "public"."user_roles" ADD CONSTRAINT "user_roles_id_fkey" FOREIGN KEY ("id") REFERENCES "auth"."users"("id") ON DELETE CASCADE;
ALTER TABLE ONLY "public"."user_usage" ADD CONSTRAINT "user_usage_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "public"."user_roles"("id") ON DELETE CASCADE;

-- 创建行级安全策略
CREATE POLICY "AUTHENTICATED" ON "public"."chats" TO "authenticated" USING (("userId" = "auth"."uid"())) WITH CHECK (("userId" = "auth"."uid"()));
CREATE POLICY "AUTHENTICATED" ON "public"."document_files" TO "authenticated" USING ((EXISTS ( SELECT 1 FROM "public"."user_roles" WHERE (("user_roles"."id" = "document_files"."owner") AND ("user_roles"."id" = "auth"."uid"()))))) WITH CHECK ((EXISTS ( SELECT 1 FROM "public"."user_roles" WHERE (("user_roles"."id" = "document_files"."owner") AND ("user_roles"."id" = "auth"."uid"())))));
CREATE POLICY "AUTHENTICATED" ON "public"."embeddings" TO "authenticated" USING ((EXISTS ( SELECT 1 FROM "public"."document_files" WHERE (("document_files"."id" = "embeddings"."file_id") AND (EXISTS ( SELECT 1 FROM "public"."user_roles" WHERE (("user_roles"."id" = "document_files"."owner") AND ("user_roles"."id" = "auth"."uid"())))))))) WITH CHECK ((EXISTS ( SELECT 1 FROM "public"."document_files" WHERE (("document_files"."id" = "embeddings"."file_id") AND (EXISTS ( SELECT 1 FROM "public"."user_roles" WHERE (("user_roles"."id" = "document_files"."owner") AND ("user_roles"."id" = "auth"."uid"()))))))));
CREATE POLICY "AUTHENTICATED" ON "public"."messages" TO "authenticated" USING ((EXISTS ( SELECT 1 FROM "public"."chats" WHERE (("chats"."id" = "messages"."chatId") AND ("chats"."userId" = "auth"."uid"()))))) WITH CHECK ((EXISTS ( SELECT 1 FROM "public"."chats" WHERE (("chats"."id" = "messages"."chatId") AND ("chats"."userId" = "auth"."uid"())))));
